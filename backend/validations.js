...
export const TaskCreateValidation = [
    body('title', 'Введите норм заголовок').isLength({min: 3}).isString(),
    body('text', 'Введите больше 10 символов').isLength({min: 10}).isString(),
    body('type', 'Ты тютор или ученик??').isString(),
    body('budget_type', 'Укажи тип оплаты').isLength({min: 2}).isString(),
